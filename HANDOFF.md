...
- **Last updated:** 2026-07-04 (v0.3.41 loop — CI doctor example workflow)
...
| Published to npm | ✅ `claude-loadout@0.3.41` live. Local `npm publish` uses `~/.npmrc` token. **`prepublishOnly` runs `npm test`.** GitHub Actions `publish` workflow needs repo secret `NPM_TOKEN` — without it the job warns and skips. See CONTRIBUTING § Release. CI template: `examples/ci-doctor.yml`. |
