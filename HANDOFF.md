...
- **Last updated:** 2026-07-04 (v0.3.21 loop — richer JSON + doctor summary)
...
| Published to npm | ✅ `claude-loadout@0.3.21` live. Local `npm publish` uses `~/.npmrc` token. **GitHub Actions `publish` workflow** needs repo secret `NPM_TOKEN` (granular publish token) — without it the job warns and skips (not a hard fail). See CONTRIBUTING § Release. |
