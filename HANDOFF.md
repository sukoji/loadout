...
- **Last updated:** 2026-07-05 (v0.3.83 loop — Go/Rust/turbo/CI fixtures)
...
| Published to npm | ✅ `claude-loadout@0.3.83` live. Local `npm publish` uses `~/.npmrc` token. **`prepublishOnly` runs `npm test`.** GitHub Actions `publish` workflow needs repo secret `NPM_TOKEN` — without it the job warns and skips. See CONTRIBUTING § Release. CI: `loadout-doctor.yml` uses `doctor --json --require-healthy`. Scan: full domain coverage across all 10 domains. |
