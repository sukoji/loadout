...
- **Last updated:** 2026-07-04 (v0.3.32 loop — doctor profile JSON)
...
| Published to npm | ✅ `claude-loadout@0.3.32` live. Local `npm publish` uses `~/.npmrc` token. **`prepublishOnly` runs `npm test`.** GitHub Actions `publish` workflow needs repo secret `NPM_TOKEN` — without it the job warns and skips. See CONTRIBUTING § Release. |
