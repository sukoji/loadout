...
- **Last updated:** 2026-07-04 (v0.3.58 loop — Astro/Remix/SvelteKit/Deno scan)
...
| Published to npm | ✅ `claude-loadout@0.3.58` live. Local `npm publish` uses `~/.npmrc` token. **`prepublishOnly` runs `npm test`.** GitHub Actions `publish` workflow needs repo secret `NPM_TOKEN` — without it the job warns and skips. See CONTRIBUTING § Release. CI: `loadout-doctor.yml` uses `doctor --json --require-healthy`. Scan: monorepo + Astro/Remix/SvelteKit/Deno. |
