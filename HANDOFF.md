...
- **Last updated:** 2026-07-05 (v0.3.62 loop — Deno frontend + VitePress docs)
...
| Published to npm | ✅ `claude-loadout@0.3.62` live. Local `npm publish` uses `~/.npmrc` token. **`prepublishOnly` runs `npm test`.** GitHub Actions `publish` workflow needs repo secret `NPM_TOKEN` — without it the job warns and skips. See CONTRIBUTING § Release. CI: `loadout-doctor.yml` uses `doctor --json --require-healthy`. Scan: Deno frontend + VitePress docs; Nuxt/Solid/Qwik/Bun stacks. |
