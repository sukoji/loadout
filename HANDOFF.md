...
- **Last updated:** 2026-07-05 (v0.3.69 loop — Node/Flask backend recommend fixtures)
...
| Published to npm | ✅ `claude-loadout@0.3.69` live. Local `npm publish` uses `~/.npmrc` token. **`prepublishOnly` runs `npm test`.** GitHub Actions `publish` workflow needs repo secret `NPM_TOKEN` — without it the job warns and skips. See CONTRIBUTING § Release. CI: `loadout-doctor.yml` uses `doctor --json --require-healthy`. Scan: full backend stack coverage (Node/Python/Go/Rust/Java/Elixir/PHP). |
