...
- **Last updated:** 2026-07-04 (v0.3.43 loop — dogfood .mcp.json)
...
| Published to npm | ✅ `claude-loadout@0.3.43` live. Local `npm publish` uses `~/.npmrc` token. **`prepublishOnly` runs `npm test`.** GitHub Actions `publish` workflow needs repo secret `NPM_TOKEN` — without it the job warns and skips. See CONTRIBUTING § Release. CI: `loadout-doctor.yml` + template `examples/ci-doctor.yml`. Project `.mcp.json` dogfoods apply --suggestions. |
