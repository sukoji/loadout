...
### Loadout은 프로젝트를 보고 거기에 맞게 Claude Code를 세팅합니다.

내 스택에 맞는 MCP 서버·훅·스킬이 뭔지 골라서, 각각 왜 필요한지 이유와 함께 짧은 랭킹으로 보여주고, 고른 것만
...
> **한 줄 요약:** repo를 가리키면, 그 repo에 맞는 Claude Code 확장을 추천하고, 고른 걸 적용해줍니다.

## 문제

Claude Code 생태계에는 수천 개의 스킬·MCP 서버·훅·플러그인이 있고, 수십 개의 "awesome" 리스트에 흩어져 있습니다.
발견하기엔 좋은데 딱 거기까지예요. 수백 개 항목을 읽고 내게 뭐가 맞는지 추측한 다음, 설치 명령을 하나씩 복붙해야
하니까요.

Loadout은 그 나머지 절반을 합니다. 프로젝트를 가리키면 그 프로젝트에 뭘 깔아야 하는지 알려주고, 세팅까지 해줍니다.
...
프로젝트를 스캔(언어·프레임워크·CI·기존 설정)해서 하나 이상의 도메인에 매칭하고, 각 항목에 왜 필요한지 한 줄
이유를 붙여 짧은 로드아웃으로 정리해 보여줍니다. 고른 걸 골라주면 설정을 직접 작성하는데, `.mcp.json`과
`.claude/settings.json`에 병합만 하고 기존 설정을 덮어쓰지 않으며, 아직 채워야 할 토큰이 있으면 알려줍니다.
...
| `doctor --require-healthy` | `summary.healthy`가 아니면 exit 1 (선택 플러그인은 healthy로 취급) |
| `signals` / `signals --json` | 감지된 시그널 + 매칭 도메인 출력 (추천 디버그용) |
