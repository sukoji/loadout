...
const workspacesMonorepo = new Set(["always", "monorepo", "package.json", ".git"]);
const wsTop = topNames(workspacesMonorepo);
assert("workspaces monorepo includes filesystem or git", wsTop.includes("filesystem") || wsTop.includes("git"), wsTop.join(", "));
assert("workspaces monorepo includes guard-dangerous-bash", wsTop.includes("guard-dangerous-bash"), wsTop.join(", "));
assert("workspaces monorepo excludes playwright", !wsTop.includes("playwright"), wsTop.join(", "));
const dockerOnly = new Set(["always", "dockerfile", ".git"]);
const dockerOnlyTop = topNames(dockerOnly);
assert("dockerfile-only devops includes git or github", dockerOnlyTop.includes("git") || dockerOnlyTop.includes("github"), dockerOnlyTop.join(", "));
assert("dockerfile-only devops includes guard-dangerous-bash", dockerOnlyTop.includes("guard-dangerous-bash"), dockerOnlyTop.join(", "));
assert("dockerfile-only devops excludes playwright", !dockerOnlyTop.includes("playwright"), dockerOnlyTop.join(", "));
const ansibleOnly = new Set(["always", "ansible", ".git"]);
const ansibleOnlyTop = topNames(ansibleOnly);
assert("ansible-only devops includes git or github", ansibleOnlyTop.includes("git") || ansibleOnlyTop.includes("github"), ansibleOnlyTop.join(", "));
assert("ansible-only devops includes guard-dangerous-bash", ansibleOnlyTop.includes("guard-dangerous-bash"), ansibleOnlyTop.join(", "));
assert("ansible-only devops excludes playwright", !ansibleOnlyTop.includes("playwright"), ansibleOnlyTop.join(", "));
const docxOnly = new Set(["always", ".docx", ".git"]);
const docxTop = topNames(docxOnly);
assert("docx-only includes office-docs", docxTop.includes("office-docs"), docxTop.join(", "));
assert("docx-only excludes playwright", !docxTop.includes("playwright"), docxTop.join(", "));
const postgresBackend = new Set(["always", "package.json", "express", "postgres", ".git"]);
const postgresBeTop = topNames(postgresBackend);
